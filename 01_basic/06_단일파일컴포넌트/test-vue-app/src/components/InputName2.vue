...
export default {
  ...
  // 이벤트 유효성 검증 - 배열 형태로 이벤트명 작성
  // 1. 배열 형태로 이벤트명 작성: 등록된 이벤트가 발생하는지 확인하는 이벤트명 확인
  // nameChanged 이벤트 발신 허용, 다른 이벤트는 경고 출력
  // emits: ['nameChanged555'],

  // 2. 객체 형태로 이벤트명 작성: 이벤트명 + eventArgs 유효성 여부도 확인
  emits: {
    nameChanged: (e) => {
      // e: 이벤트 발신 시 전달된 Argument

      // 유효성 검사 코드 작성
      // (true 반환-> 이벤트 발신 허용, false 반환-> 이벤트 발신 거부+경고띄움)
